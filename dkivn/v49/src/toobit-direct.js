import {buildV2JsonRequest,buildSignedQuery,normalizeResponse} from './v2-transport.js';
...
export class ToobitDirectClient {
  ...
  place(order){return this.#send(this.#j('/api/v2/futures/order',{symbol:order.symbol,side:order.side,positionSide:order.positionSide,type:'LIMIT',newClientOrderId:order.clientOrderId,valueQuantity:String(order.valueQuantity),price:String(order.price),timeInForce:'POST_ONLY'}));}
  update(order){return this.#send(this.#j('/api/v2/futures/order/update',{origClientOrderId:order.origClientOrderId||order.clientOrderId,newClientOrderId:order.clientOrderId,valueQuantity:String(order.valueQuantity),price:String(order.price),category:'USDT'}));}
