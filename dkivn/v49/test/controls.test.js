...
test('flash close endpoint requires explicit confirmation and stops maker first',()=>{assert.ok(server.includes("'/position/flash-close'"));assert.ok(server.includes("'CLOSE_ALL_BTC'"));assert.ok(server.includes("runtime.stop('MANUAL_FLAT')"));assert.ok(server.includes('runtime.client.flashClose'))});
