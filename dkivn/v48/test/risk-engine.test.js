...
test('readiness remains DISARMED while live execution is disabled',()=>{ const r=evaluateReadiness(base,{...config,liveEnabled:false}); assert.equal(r.ok,false); assert.equal(r.state,'DISARMED'); assert.equal(r.reason,'LIVE_DISABLED'); assert.equal(r.allowBid,false); assert.equal(r.allowAsk,false); });
